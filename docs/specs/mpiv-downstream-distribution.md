...
- Seam: the immutable `bb-app.tgz`, its provenance manifest, and the artifact-bound deployment approval marker.
...
- Metric: each prepared build records source commit, upstream base, version, protocol version, checksum, creation time, and custom commit count; each production attempt records its verification result in the release directory and Hub worker state.
...
6. The Hub worker accepts only the newest build with a matching approval marker and deploys it through the canonical rollback-first installer.
...
10. The documentation distinguishes server/web, host-daemon, and desktop release behavior and gives the upstream intake, release, rollback, and removal procedures.
...
- Any server-daemon wire change increments `HOST_DAEMON_PROTOCOL_VERSION`; enrolled machines then update from the Hub's exact package.
...
6. `scripts/mpiv-hub-worker.mjs:57`, `scripts/mpiv-hub-worker.mjs:201`, and `scripts/mpiv-hub/bb-mpiv-deploy-worker.timer:4` — `selects only the newest approved build and runs the canonical installer on mpiv-hub`
