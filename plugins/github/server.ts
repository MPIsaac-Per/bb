...
export default async function plugin(bb: BbPluginApi) {
  ...
  bb.rpc.register(githubRpcContract, {
    ...
    async listOwners(input) {
      ...
      const extra = parseExtraRepos(extraRepos);
      const extraSet = new Set(extra.valid);
      ...
      return {
        ...
        invalidExtraRepos: extra.invalid,
      };
    },
    ...
    async run(argv) {
      ...
      try {
        ...
        if (sub === "repos") {
          ...
          if (repos.length === 0) {
            return { exitCode: 0, stdout: "No tracked repos. Attach a project with a GitHub remote, pick repositories in Settings, or set extraRepos." };
          }
          return {
            exitCode: 0,
            stdout: repos
