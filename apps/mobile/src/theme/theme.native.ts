/**
 * GENERATED FILE — run pnpm --filter @bb/mobile theme:generate
 *
 * Source: apps/app/src/components/ui/theme.css and the built-in palettes in
 * apps/app/src/lib/themes/*.ts, replayed through the web cascade per palette
 * and mode by apps/mobile/scripts/generate-native-theme.ts.
 *
 * `var()` is substituted textually; `color-mix(in oklch|oklab, …)` is
 * evaluated like Chrome (premultiplied alpha, shorter hue arc, converted
 * near-achromatic operands lose their hue). Opaque results are `#rrggbb`,
 * translucent ones `rgba(r, g, b, a)`. Typography uses the coarse-pointer
 * (touch) sizes as the base scale, in CSS pixels.
 *
 * Tokens deliberately left out (edit the generator to add them):
 *   --bb-sidebar-row-height: dimension (1.75rem)
 *   --bb-sidebar-row-height-coarse: dimension (2.5rem)
 *   --diffs-dark: @pierre/diffs bridge; defined per mode only
 *   --diffs-dark-addition-color: @pierre/diffs bridge; defined per mode only
 *   --diffs-dark-bg: @pierre/diffs bridge; defined per mode only
 *   --diffs-dark-deletion-color: @pierre/diffs bridge; defined per mode only
 *   --diffs-light: @pierre/diffs bridge; defined per mode only
 *   --diffs-light-addition-color: @pierre/diffs bridge; defined per mode only
 *   --diffs-light-bg: @pierre/diffs bridge; defined per mode only
 *   --diffs-light-deletion-color: @pierre/diffs bridge; defined per mode only
 *   --font-mono: font stack
 *   --font-sans: font stack
 *   --font-serif: font stack
 *   --icon-stroke-width: dimension (1.75)
 *   --pill-shadow: box-shadow
 *   --pill-surface: gradient
 *   --pill-surface-selected: gradient
 *   --radius: emitted as nativeRadii
 *   --resource-source-shelf-action-block: computed dimension
 *   --resource-source-shelf-action-inline: computed dimension
 *   --resource-source-shelf-card-action-gap: computed dimension
 *   --resource-source-shelf-card-hover-shadow: box-shadow
 *   --resource-source-shelf-fade-ramp: computed dimension
 *   --resource-source-shelf-inset: computed dimension
 *   --resource-source-shelf-item-gap: computed dimension
 *   --resource-source-shelf-item-width: dimension (31.25%)
 *   --resource-source-shelf-label-gap: computed dimension
 *   --resource-source-shelf-section-gap: computed dimension
 *   --resource-source-shelf-shadow-bleed: computed dimension
 *   --resource-source-shelf-shadow-left-bleed: computed dimension
 *   --shadow: box-shadow
 *   --shadow-2xl: box-shadow
 *   --shadow-2xs: box-shadow
 *   --shadow-blur: dimension (0px)
 *   --shadow-lg: box-shadow
 *   --shadow-lift: box-shadow
 *   --shadow-md: box-shadow
 *   --shadow-opacity: dimension (0.15)
 *   --shadow-sm: box-shadow
 *   --shadow-spread: dimension (0px)
 *   --shadow-x: dimension (0px)
 *   --shadow-xl: box-shadow
 *   --shadow-xs: box-shadow
 *   --shadow-y: dimension (2px)
 *   --spacing: dimension (0.25rem)
 *   --tracking-normal: dimension (0em)
 */
import type { BuiltInThemeId } from "@bb/domain";

/**
 * theme.css custom-property color tokens, keyed by camelCase name. Every
 * value is a React Native color string.
 */
export interface NativeThemeTokens {
  accent: string;
  accentForeground: string;
  ansi0: string;
  ansi1: string;
  ansi10: string;
  ansi11: string;
  ansi12: string;
  ansi13: string;
  ansi14: string;
  ansi15: string;
  ansi2: string;
  ansi3: string;
  ansi4: string;
  ansi5: string;
  ansi6: string;
  ansi7: string;
  ansi8: string;
  ansi9: string;
  ansiBgFg0: string;
  ansiBgFg1: string;
  ansiBgFg10: string;
  ansiBgFg11: string;
  ansiBgFg12: string;
  ansiBgFg13: string;
  ansiBgFg14: string;
  ansiBgFg15: string;
  ansiBgFg2: string;
  ansiBgFg3: string;
  ansiBgFg4: string;
  ansiBgFg5: string;
  ansiBgFg6: string;
  ansiBgFg7: string;
  ansiBgFg8: string;
  ansiBgFg9: string;
  attention: string;
  background: string;
  border: string;
  borderHairline: string;
  borderSeam: string;
  borderSeamVertical: string;
  canvas: string;
  card: string;
  cardForeground: string;
  destructive: string;
  destructiveForeground: string;
  destructiveText: string;
  diffAdded: string;
  diffRemoved: string;
  fileAccent: string;
  foreground: string;
  ink: string;
  input: string;
  muted: string;
  mutedForeground: string;
  pillForeground: string;
  pillIcon: string;
  pillSurfaceBorder: string;
  pillSurfaceSelectedBorder: string;
  popover: string;
  popoverForeground: string;
  prMerged: string;
  primary: string;
  primaryForeground: string;
  readbackForeground: string;
  resourceSourceShelfCardHoverBorder: string;
  ring: string;
  secondary: string;
  secondaryForeground: string;
  shadowColor: string;
  sidebar: string;
  sidebarAccent: string;
  sidebarAccentForeground: string;
  sidebarBorder: string;
  sidebarForeground: string;
  sidebarRing: string;
  sidebarSearchMatch: string;
  sidebarSearchMatchBorder: string;
  stateActive: string;
  stateHover: string;
  subtleForeground: string;
  success: string;
  successForeground: string;
  surfaceAttention: string;
  surfaceDestructive: string;
  surfaceDestructiveBorder: string;
  surfaceRaised: string;
  surfaceRaisedSolid: string;
  surfaceRecessed: string;
  surfaceRecessedSoftSolid: string;
  surfaceRecessedSolid: string;
  surfaceScrim: string;
  surfaceSelected: string;
  surfaceSelectedBorder: string;
  timelineAccent: string;
  versionUpgrade: string;
  warning: string;
  warningText: string;
}

export interface NativeThemeModes {
  light: NativeThemeTokens;
  dark: NativeThemeTokens;
}

export const nativeThemes: Record<BuiltInThemeId, NativeThemeModes> = {
  "anysphere-dark": {
    light: {
      accent: "#e7e7e7",
      accentForeground: "#141414",
      ansi0: "#141414",
      ansi1: "#be1744",
      ansi10: "#00854c",
      ansi11: "#a46700",
      ansi12: "#2778c1",
      ansi13: "#b54e90",
      ansi14: "#3b7e84",
      ansi15: "#ffffff",
      ansi2: "#007041",
      ansi3: "#8b5700",
      ansi4: "#0064b0",
      ansi5: "#92156a",
      ansi6: "#176c74",
      ansi7: "#fcfcfc",
      ansi8: "#767676",
      ansi9: "#ce405b",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#ffffff",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#ffffff",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#ffffff",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#ffffff",
      ansiBgFg3: "#ffffff",
      ansiBgFg4: "#ffffff",
      ansiBgFg5: "#ffffff",
      ansiBgFg6: "#ffffff",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#000000",
      attention: "#8b5700",
      background: "#fcfcfc",
      border: "#d7d7d7",
      borderHairline: "#d5d5d5",
      borderSeam: "#e3e3e3",
      borderSeamVertical: "#e3e3e3",
      canvas: "#fcfcfc",
      card: "#fcfcfc",
      cardForeground: "#141414",
      destructive: "#be1744",
      destructiveForeground: "#ffffff",
      destructiveText: "#be1744",
      diffAdded: "#007041",
      diffRemoved: "#be1744",
      fileAccent: "#2778c1",
      foreground: "#141414",
      ink: "#141414",
      input: "#afafaf",
      muted: "#dfdfdf",
      mutedForeground: "rgba(20, 20, 20, 0.741)",
      pillForeground: "#141414",
      pillIcon: "#141414",
      pillSurfaceBorder: "#d7d7d7",
      pillSurfaceSelectedBorder: "#cacaca",
      popover: "#fcfcfc",
      popoverForeground: "#141414",
      prMerged: "#92156a",
      primary: "#2778c1",
      primaryForeground: "#fcfcfc",
      readbackForeground: "rgba(20, 20, 20, 0.6)",
      resourceSourceShelfCardHoverBorder: "#bdbdbd",
      ring: "#2778c1",
      secondary: "#e7e7e7",
      secondaryForeground: "#141414",
      shadowColor: "rgba(51, 51, 51, 0.1)",
      sidebar: "#f3f3f3",
      sidebarAccent: "#dcdcdc",
      sidebarAccentForeground: "#141414",
      sidebarBorder: "#cfcfcf",
      sidebarForeground: "#141414",
      sidebarRing: "#2778c1",
      sidebarSearchMatch: "#f6ecd6",
      sidebarSearchMatchBorder: "#f1e2bc",
      stateActive: "rgba(20, 20, 20, 0.118)",
      stateHover: "rgba(20, 20, 20, 0.059)",
      subtleForeground: "rgba(20, 20, 20, 0.6)",
      success: "#007041",
      successForeground: "#1a3b28",
      surfaceAttention: "rgba(139, 87, 0, 0.14)",
      surfaceDestructive: "rgba(190, 23, 68, 0.06)",
      surfaceDestructiveBorder: "rgba(190, 23, 68, 0.25)",
      surfaceRaised: "rgba(20, 20, 20, 0.025)",
      surfaceRaisedSolid: "#f5f5f5",
      surfaceRecessed: "rgba(20, 20, 20, 0.06)",
      surfaceRecessedSoftSolid: "#f1f1f1",
      surfaceRecessedSolid: "#ececec",
      surfaceScrim: "rgba(252, 252, 252, 0.92)",
      surfaceSelected: "rgba(39, 120, 193, 0.16)",
      surfaceSelectedBorder: "rgba(39, 120, 193, 0.35)",
      timelineAccent: "#2778c1",
      versionUpgrade: "#1b1b1b",
      warning: "#a46700",
      warningText: "#8b5700",
    },
    dark: {
      accent: "#2f2f2f",
      accentForeground: "#f0f0f0",
      ansi0: "#242424",
      ansi1: "#fc6b83",
      ansi10: "#70b489",
      ansi11: "#f1b467",
      ansi12: "#87a6c4",
      ansi13: "#b48ead",
      ansi14: "#88c0d0",
      ansi15: "#ffffff",
      ansi2: "#3fa266",
      ansi3: "#d2943e",
      ansi4: "#81a1c1",
      ansi5: "#b48ead",
      ansi6: "#88c0d0",
      ansi7: "#f0f0f0",
      ansi8: "#929292",
      ansi9: "#fc6b83",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#000000",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#000000",
      ansiBgFg9: "#000000",
      attention: "#d2943e",
      background: "#181818",
      border: "#3b3b3b",
      borderHairline: "#3e3e3e",
      borderSeam: "#2c2c2c",
      borderSeamVertical: "#2c2c2c",
      canvas: "#181818",
      card: "#181818",
      cardForeground: "#f0f0f0",
      destructive: "#fc6b83",
      destructiveForeground: "#ffffff",
      destructiveText: "#fc6b83",
      diffAdded: "#3fa266",
      diffRemoved: "#fc6b83",
      fileAccent: "#88c0d0",
      foreground: "#f0f0f0",
      ink: "#f0f0f0",
      input: "#565656",
      muted: "#353535",
      mutedForeground: "rgba(240, 240, 240, 0.741)",
      pillForeground: "#f0f0f0",
      pillIcon: "#f0f0f0",
      pillSurfaceBorder: "#3b3b3b",
      pillSurfaceSelectedBorder: "#474747",
      popover: "#181818",
      popoverForeground: "#f0f0f0",
      prMerged: "#b48ead",
      primary: "#81a1c1",
      primaryForeground: "#191c22",
      readbackForeground: "rgba(240, 240, 240, 0.6)",
      resourceSourceShelfCardHoverBorder: "#505050",
      ring: "#81a1c1",
      secondary: "#2f2f2f",
      secondaryForeground: "#f0f0f0",
      shadowColor: "rgba(0, 0, 0, 0.4)",
      sidebar: "#141414",
      sidebarAccent: "#242424",
      sidebarAccentForeground: "#f0f0f0",
      sidebarBorder: "#292929",
      sidebarForeground: "#f0f0f0",
      sidebarRing: "#81a1c1",
      sidebarSearchMatch: "#3a3425",
      sidebarSearchMatchBorder: "#52482d",
      stateActive: "rgba(240, 240, 240, 0.225)",
      stateHover: "rgba(240, 240, 240, 0.138)",
      subtleForeground: "rgba(240, 240, 240, 0.6)",
      success: "#70b489",
      successForeground: "#b7d5c1",
      surfaceAttention: "rgba(210, 148, 62, 0.12)",
      surfaceDestructive: "rgba(252, 107, 131, 0.08)",
      surfaceDestructiveBorder: "rgba(252, 107, 131, 0.3)",
      surfaceRaised: "rgba(240, 240, 240, 0.025)",
      surfaceRaisedSolid: "#1c1c1c",
      surfaceRecessed: "rgba(240, 240, 240, 0.06)",
      surfaceRecessedSoftSolid: "#1f1f1f",
      surfaceRecessedSolid: "#222222",
      surfaceScrim: "rgba(24, 24, 24, 0.92)",
      surfaceSelected: "rgba(129, 161, 193, 0.12)",
      surfaceSelectedBorder: "rgba(129, 161, 193, 0.35)",
      timelineAccent: "#88c0d0",
      versionUpgrade: "#e6e6e6",
      warning: "#f1b467",
      warningText: "#f1b467",
    },
  },
  catppuccin: {
    light: {
      accent: "#e2e3e9",
      accentForeground: "#4c4f69",
      ansi0: "#5c5f77",
      ansi1: "#d20f39",
      ansi10: "#40a02b",
      ansi11: "#df8e1d",
      ansi12: "#1e66f5",
      ansi13: "#ea76cb",
      ansi14: "#179299",
      ansi15: "#bcc0cc",
      ansi2: "#40a02b",
      ansi3: "#df8e1d",
      ansi4: "#1e66f5",
      ansi5: "#ea76cb",
      ansi6: "#179299",
      ansi7: "#acb0be",
      ansi8: "#6c6f85",
      ansi9: "#d20f39",
      ansiBgFg0: "#eff1f5",
      ansiBgFg1: "#eff1f5",
      ansiBgFg10: "#eff1f5",
      ansiBgFg11: "#11111b",
      ansiBgFg12: "#eff1f5",
      ansiBgFg13: "#11111b",
      ansiBgFg14: "#eff1f5",
      ansiBgFg15: "#11111b",
      ansiBgFg2: "#eff1f5",
      ansiBgFg3: "#11111b",
      ansiBgFg4: "#eff1f5",
      ansiBgFg5: "#11111b",
      ansiBgFg6: "#eff1f5",
      ansiBgFg7: "#11111b",
      ansiBgFg8: "#eff1f5",
      ansiBgFg9: "#eff1f5",
      attention: "#fe640b",
      background: "#eff1f5",
      border: "#d7d8e0",
      borderHairline: "#d6d7df",
      borderSeam: "#dfe0e7",
      borderSeamVertical: "#dfe0e7",
      canvas: "#eff1f5",
      card: "#eff1f5",
      cardForeground: "#4c4f69",
      destructive: "#d20f39",
      destructiveForeground: "#eff1f5",
      destructiveText: "#d20f39",
      diffAdded: "#40a02b",
      diffRemoved: "#d20f39",
      fileAccent: "#1e66f5",
      foreground: "#4c4f69",
      ink: "#4c4f69",
      input: "#bcbeca",
      muted: "#dcdde5",
      mutedForeground: "#5c5f77",
      pillForeground: "#4c4f69",
      pillIcon: "#4c4f69",
      pillSurfaceBorder: "#d7d8e0",
      pillSurfaceSelectedBorder: "#cecfd9",
      popover: "#eff1f5",
      popoverForeground: "#4c4f69",
      prMerged: "#8839ef",
      primary: "#8839ef",
      primaryForeground: "#eff1f5",
      readbackForeground: "#6c6f85",
      resourceSourceShelfCardHoverBorder: "#c6c7d2",
      ring: "#8839ef",
      secondary: "#e2e3e9",
      secondaryForeground: "#4c4f69",
      shadowColor: "rgba(51, 51, 51, 0.1)",
      sidebar: "#ecedf2",
      sidebarAccent: "#e2e3e9",
      sidebarAccentForeground: "#4c4f69",
      sidebarBorder: "#d7d8e0",
      sidebarForeground: "#4c4f69",
      sidebarRing: "#8839ef",
      sidebarSearchMatch: "#ece4d1",
      sidebarSearchMatchBorder: "#eadbb8",
      stateActive: "rgba(76, 79, 105, 0.118)",
      stateHover: "rgba(76, 79, 105, 0.059)",
      subtleForeground: "#8c8fa1",
      success: "#40a02b",
      successForeground: "#007790",
      surfaceAttention: "rgba(254, 100, 11, 0.14)",
      surfaceDestructive: "rgba(210, 15, 57, 0.06)",
      surfaceDestructiveBorder: "rgba(210, 15, 57, 0.25)",
      surfaceRaised: "rgba(76, 79, 105, 0.025)",
      surfaceRaisedSolid: "#ebedf1",
      surfaceRecessed: "rgba(76, 79, 105, 0.06)",
      surfaceRecessedSoftSolid: "#e8e9ef",
      surfaceRecessedSolid: "#e4e6ec",
      surfaceScrim: "rgba(239, 241, 245, 0.92)",
      surfaceSelected: "rgba(136, 57, 239, 0.16)",
      surfaceSelectedBorder: "rgba(136, 57, 239, 0.35)",
      timelineAccent: "#1e66f5",
      versionUpgrade: "#52556e",
      warning: "#df8e1d",
      warningText: "#b8730a",
    },
    dark: {
      accent: "#313244",
      accentForeground: "#cdd6f4",
      ansi0: "#45475a",
      ansi1: "#f38ba8",
      ansi10: "#a6e3a1",
      ansi11: "#f9e2af",
      ansi12: "#89b4fa",
      ansi13: "#f5c2e7",
      ansi14: "#94e2d5",
      ansi15: "#a6adc8",
      ansi2: "#a6e3a1",
      ansi3: "#f9e2af",
      ansi4: "#89b4fa",
      ansi5: "#f5c2e7",
      ansi6: "#94e2d5",
      ansi7: "#bac2de",
      ansi8: "#585b70",
      ansi9: "#f38ba8",
      ansiBgFg0: "#cdd6f4",
      ansiBgFg1: "#11111b",
      ansiBgFg10: "#11111b",
      ansiBgFg11: "#11111b",
      ansiBgFg12: "#11111b",
      ansiBgFg13: "#11111b",
      ansiBgFg14: "#11111b",
      ansiBgFg15: "#11111b",
      ansiBgFg2: "#11111b",
      ansiBgFg3: "#11111b",
      ansiBgFg4: "#11111b",
      ansiBgFg5: "#11111b",
      ansiBgFg6: "#11111b",
      ansiBgFg7: "#11111b",
      ansiBgFg8: "#cdd6f4",
      ansiBgFg9: "#11111b",
      attention: "#fab387",
      background: "#1e1e2e",
      border: "#3c3d50",
      borderHairline: "#3e4053",
      borderSeam: "#2e2f41",
      borderSeamVertical: "#2e2f41",
      canvas: "#1e1e2e",
      card: "#1e1e2e",
      cardForeground: "#cdd6f4",
      destructive: "#f38ba8",
      destructiveForeground: "#1e1e2e",
      destructiveText: "#f38ba8",
      diffAdded: "#a6e3a1",
      diffRemoved: "#f38ba8",
      fileAccent: "#89b4fa",
      foreground: "#cdd6f4",
      ink: "#cdd6f4",
      input: "#515368",
      muted: "#36374a",
      mutedForeground: "#bac2de",
      pillForeground: "#cdd6f4",
      pillIcon: "#cdd6f4",
      pillSurfaceBorder: "#3c3d50",
      pillSurfaceSelectedBorder: "#45475b",
      popover: "#1e1e2e",
      popoverForeground: "#cdd6f4",
      prMerged: "#cba6f7",
      primary: "#cba6f7",
      primaryForeground: "#1e1e2e",
      readbackForeground: "#a6adc8",
      resourceSourceShelfCardHoverBorder: "#4d4f64",
      ring: "#cba6f7",
      secondary: "#313244",
      secondaryForeground: "#cdd6f4",
      shadowColor: "rgba(0, 0, 0, 0.4)",
      sidebar: "#242535",
      sidebarAccent: "#303143",
      sidebarAccentForeground: "#cdd6f4",
      sidebarBorder: "#3a3b4e",
      sidebarForeground: "#cdd6f4",
      sidebarRing: "#cba6f7",
      sidebarSearchMatch: "#3f3a3a",
      sidebarSearchMatchBorder: "#574d41",
      stateActive: "rgba(205, 214, 244, 0.225)",
      stateHover: "rgba(205, 214, 244, 0.138)",
      subtleForeground: "#7f849c",
      success: "#a6e3a1",
      successForeground: "#9ce1f1",
      surfaceAttention: "rgba(250, 179, 135, 0.12)",
      surfaceDestructive: "rgba(243, 139, 168, 0.08)",
      surfaceDestructiveBorder: "rgba(243, 139, 168, 0.3)",
      surfaceRaised: "rgba(205, 214, 244, 0.025)",
      surfaceRaisedSolid: "#222232",
      surfaceRecessed: "rgba(205, 214, 244, 0.06)",
      surfaceRecessedSoftSolid: "#242435",
      surfaceRecessedSolid: "#272738",
      surfaceScrim: "rgba(30, 30, 46, 0.92)",
      surfaceSelected: "rgba(203, 166, 247, 0.12)",
      surfaceSelectedBorder: "rgba(203, 166, 247, 0.35)",
      timelineAccent: "#89b4fa",
      versionUpgrade: "#c5ceeb",
      warning: "#f9e2af",
      warningText: "#f9e2af",
    },
  },
  chatgpt: {
    light: {
      accent: "#e7e5e6",
      accentForeground: "#2d333a",
      ansi0: "#2d333a",
      ansi1: "#c2413b",
      ansi10: "#10a37f",
      ansi11: "#bd8510",
      ansi12: "#3b7bc4",
      ansi13: "#956fdc",
      ansi14: "#1d91a8",
      ansi15: "#ffffff",
      ansi2: "#0d8f72",
      ansi3: "#9a6b05",
      ansi4: "#2563a9",
      ansi5: "#7c5ac7",
      ansi6: "#147d92",
      ansi7: "#d9d9e3",
      ansi8: "#6e7378",
      ansi9: "#dc514b",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#ffffff",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#ffffff",
      ansiBgFg3: "#ffffff",
      ansiBgFg4: "#ffffff",
      ansiBgFg5: "#ffffff",
      ansiBgFg6: "#ffffff",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#000000",
      attention: "#9a6b05",
      background: "#f7f7f8",
      border: "#dbd8d9",
      borderHairline: "#d9d6d7",
      borderSeam: "#e4e2e2",
      borderSeamVertical: "#e4e2e2",
      canvas: "#f7f7f8",
      card: "#f7f7f8",
      cardForeground: "#2d333a",
      destructive: "#c2413b",
      destructiveForeground: "#ffffff",
      destructiveText: "#b2332e",
      diffAdded: "#0d8f72",
      diffRemoved: "#c2413b",
      fileAccent: "#0d8f72",
      foreground: "#2d333a",
      ink: "#2d333a",
      input: "#bbb7b8",
      muted: "#e1dedf",
      mutedForeground: "#6d6567",
      pillForeground: "#2d333a",
      pillIcon: "#2d333a",
      pillSurfaceBorder: "#dbd8d9",
      pillSurfaceSelectedBorder: "#d0cdcd",
      popover: "#f7f7f8",
      popoverForeground: "#2d333a",
      prMerged: "#7c5ac7",
      primary: "#0d8f72",
      primaryForeground: "#ffffff",
      readbackForeground: "#787173",
      resourceSourceShelfCardHoverBorder: "#c6c2c3",
      ring: "#0d8f72",
      secondary: "#e7e5e6",
      secondaryForeground: "#2d333a",
      shadowColor: "rgba(51, 51, 51, 0.1)",
      sidebar: "#f3f2f2",
      sidebarAccent: "#e7e5e6",
      sidebarAccentForeground: "#2d333a",
      sidebarBorder: "#dbd8d9",
      sidebarForeground: "#2d333a",
      sidebarRing: "#0d8f72",
      sidebarSearchMatch: "#f2e8d3",
      sidebarSearchMatchBorder: "#eedfba",
      stateActive: "rgba(45, 51, 58, 0.118)",
      stateHover: "rgba(45, 51, 58, 0.059)",
      subtleForeground: "#847c7e",
      success: "#0d8f72",
      successForeground: "#2c5c4d",
      surfaceAttention: "rgba(154, 107, 5, 0.14)",
      surfaceDestructive: "rgba(194, 65, 59, 0.06)",
      surfaceDestructiveBorder: "rgba(194, 65, 59, 0.25)",
      surfaceRaised: "rgba(45, 51, 58, 0.025)",
      surfaceRaisedSolid: "#f1f2f3",
      surfaceRecessed: "rgba(45, 51, 58, 0.06)",
      surfaceRecessedSoftSolid: "#efedee",
      surfaceRecessedSolid: "#e9eaeb",
      surfaceScrim: "rgba(247, 247, 248, 0.92)",
      surfaceSelected: "rgba(13, 143, 114, 0.16)",
      surfaceSelectedBorder: "rgba(13, 143, 114, 0.35)",
      timelineAccent: "#0d8f72",
      versionUpgrade: "#403639",
      warning: "#b36b18",
      warningText: "#8e5412",
    },
    dark: {
      accent: "#373737",
      accentForeground: "#ececec",
      ansi0: "#2d333a",
      ansi1: "#c2413b",
      ansi10: "#10a37f",
      ansi11: "#bd8510",
      ansi12: "#3b7bc4",
      ansi13: "#956fdc",
      ansi14: "#1d91a8",
      ansi15: "#ffffff",
      ansi2: "#0d8f72",
      ansi3: "#9a6b05",
      ansi4: "#2563a9",
      ansi5: "#7c5ac7",
      ansi6: "#147d92",
      ansi7: "#d9d9e3",
      ansi8: "#6e7378",
      ansi9: "#dc514b",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#ffffff",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#ffffff",
      ansiBgFg3: "#ffffff",
      ansiBgFg4: "#ffffff",
      ansiBgFg5: "#ffffff",
      ansiBgFg6: "#ffffff",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#000000",
      attention: "#e7c45a",
      background: "#212121",
      border: "#434343",
      borderHairline: "#464646",
      borderSeam: "#343434",
      borderSeamVertical: "#343434",
      canvas: "#212121",
      card: "#212121",
      cardForeground: "#ececec",
      destructive: "#ef6a63",
      destructiveForeground: "#ffffff",
      destructiveText: "#f07a74",
      diffAdded: "#5ed3ad",
      diffRemoved: "#ef6a63",
      fileAccent: "#10a37f",
      foreground: "#ececec",
      ink: "#ececec",
      input: "#5c5c5c",
      muted: "#3d3d3d",
      mutedForeground: "#a9a9a9",
      pillForeground: "#ececec",
      pillIcon: "#ececec",
      pillSurfaceBorder: "#434343",
      pillSurfaceSelectedBorder: "#4e4e4e",
      popover: "#212121",
      popoverForeground: "#ececec",
      prMerged: "#ad8ee6",
      primary: "#10a37f",
      primaryForeground: "#081b16",
      readbackForeground: "#9c9c9c",
      resourceSourceShelfCardHoverBorder: "#575757",
      ring: "#10a37f",
      secondary: "#373737",
      secondaryForeground: "#ececec",
      shadowColor: "rgba(0, 0, 0, 0.4)",
      sidebar: "#171717",
      sidebarAccent: "#272727",
      sidebarAccentForeground: "#ececec",
      sidebarBorder: "#2c2c2c",
      sidebarForeground: "#ececec",
      sidebarRing: "#10a37f",
      sidebarSearchMatch: "#423c2d",
      sidebarSearchMatchBorder: "#5a4f34",
      stateActive: "rgba(236, 236, 236, 0.225)",
      stateHover: "rgba(236, 236, 236, 0.138)",
      subtleForeground: "#8f8f8f",
      success: "#5ed3ad",
      successForeground: "#b3e2cf",
      surfaceAttention: "rgba(231, 196, 90, 0.12)",
      surfaceDestructive: "rgba(239, 106, 99, 0.08)",
      surfaceDestructiveBorder: "rgba(239, 106, 99, 0.3)",
      surfaceRaised: "rgba(236, 236, 236, 0.025)",
      surfaceRaisedSolid: "#252525",
      surfaceRecessed: "rgba(236, 236, 236, 0.06)",
      surfaceRecessedSoftSolid: "#282828",
      surfaceRecessedSolid: "#2b2b2b",
      surfaceScrim: "rgba(33, 33, 33, 0.92)",
      surfaceSelected: "rgba(16, 163, 127, 0.12)",
      surfaceSelectedBorder: "rgba(16, 163, 127, 0.35)",
      timelineAccent: "#10a37f",
      versionUpgrade: "#e3e3e3",
      warning: "#f2a33c",
      warningText: "#f2a33c",
    },
  },
  default: {
    light: {
      accent: "#ededed",
      accentForeground: "#333333",
      ansi0: "#000000",
      ansi1: "#a11616",
      ansi10: "#197c52",
      ansi11: "#a35f00",
      ansi12: "#2666b0",
      ansi13: "#9b349b",
      ansi14: "#0f7798",
      ansi15: "#1f1f1f",
      ansi2: "#13704a",
      ansi3: "#7f6a00",
      ansi4: "#1f5ca6",
      ansi5: "#8a2f8a",
      ansi6: "#0b6f88",
      ansi7: "#3d3d3d",
      ansi8: "#3a3a3a",
      ansi9: "#d32f2f",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#ffffff",
      ansiBgFg10: "#ffffff",
      ansiBgFg11: "#ffffff",
      ansiBgFg12: "#ffffff",
      ansiBgFg13: "#ffffff",
      ansiBgFg14: "#ffffff",
      ansiBgFg15: "#ffffff",
      ansiBgFg2: "#ffffff",
      ansiBgFg3: "#ffffff",
      ansiBgFg4: "#ffffff",
      ansiBgFg5: "#ffffff",
      ansiBgFg6: "#ffffff",
      ansiBgFg7: "#ffffff",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#ffffff",
      attention: "#dc9e12",
      background: "#ffffff",
      border: "#dfdfdf",
      borderHairline: "#dedede",
      borderSeam: "#e9e9e9",
      borderSeamVertical: "#e9e9e9",
      canvas: "#ffffff",
      card: "#ffffff",
      cardForeground: "#333333",
      destructive: "#a5000f",
      destructiveForeground: "#ffffff",
      destructiveText: "#a5000f",
      diffAdded: "#005c32",
      diffRemoved: "#8d0000",
      fileAccent: "#4075aa",
      foreground: "#333333",
      ink: "#333333",
      input: "#bdbdbd",
      muted: "#e6e6e6",
      mutedForeground: "#525252",
      pillForeground: "#333333",
      pillIcon: "#333333",
      pillSurfaceBorder: "#dfdfdf",
      pillSurfaceSelectedBorder: "#d4d4d4",
      popover: "#ffffff",
      popoverForeground: "#333333",
      prMerged: "#7847d0",
      primary: "#262626",
      primaryForeground: "#ffffff",
      readbackForeground: "#5b5b5b",
      resourceSourceShelfCardHoverBorder: "#c9c9c9",
      ring: "#262626",
      secondary: "#ededed",
      secondaryForeground: "#333333",
      shadowColor: "rgba(51, 51, 51, 0.1)",
      sidebar: "#fafafa",
      sidebarAccent: "#ededed",
      sidebarAccentForeground: "#333333",
      sidebarBorder: "#dfdfdf",
      sidebarForeground: "#333333",
      sidebarRing: "#262626",
      sidebarSearchMatch: "#f8eed8",
      sidebarSearchMatchBorder: "#f3e3be",
      stateActive: "rgba(51, 51, 51, 0.118)",
      stateHover: "rgba(51, 51, 51, 0.059)",
      subtleForeground: "#636363",
      success: "#3bb974",
      successForeground: "#7a5a34",
      surfaceAttention: "rgba(220, 158, 18, 0.14)",
      surfaceDestructive: "rgba(165, 0, 15, 0.06)",
      surfaceDestructiveBorder: "rgba(165, 0, 15, 0.25)",
      surfaceRaised: "rgba(51, 51, 51, 0.025)",
      surfaceRaisedSolid: "#f9f9f9",
      surfaceRecessed: "rgba(51, 51, 51, 0.06)",
      surfaceRecessedSoftSolid: "#f5f5f5",
      surfaceRecessedSolid: "#f1f1f1",
      surfaceScrim: "rgba(255, 255, 255, 0.92)",
      surfaceSelected: "rgba(38, 38, 38, 0.16)",
      surfaceSelectedBorder: "rgba(38, 38, 38, 0.35)",
      timelineAccent: "#4075aa",
      versionUpgrade: "#3a3a3a",
      warning: "#eb7c33",
      warningText: "#b0540e",
    },
    dark: {
      accent: "#282828",
      accentForeground: "#c1c1c1",
      ansi0: "#858585",
      ansi1: "#d85e5e",
      ansi10: "#23d18b",
      ansi11: "#f5f543",
      ansi12: "#5aaaf2",
      ansi13: "#d670d6",
      ansi14: "#29b8db",
      ansi15: "#ffffff",
      ansi2: "#0dbc79",
      ansi3: "#e5e510",
      ansi4: "#3c88dc",
      ansi5: "#c85ac8",
      ansi6: "#11a8cd",
      ansi7: "#e5e5e5",
      ansi8: "#9a9a9a",
      ansi9: "#ff6f6f",
      ansiBgFg0: "#000000",
      ansiBgFg1: "#000000",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#000000",
      ansiBgFg9: "#000000",
      attention: "#f0b135",
      background: "#151515",
      border: "#313131",
      borderHairline: "#343434",
      borderSeam: "#252525",
      borderSeamVertical: "#252525",
      canvas: "#151515",
      card: "#151515",
      cardForeground: "#c1c1c1",
      destructive: "#cc323d",
      destructiveForeground: "#ffffff",
      destructiveText: "#e06062",
      diffAdded: "#00d594",
      diffRemoved: "#ff696d",
      fileAccent: "#79a9db",
      foreground: "#c1c1c1",
      ink: "#c1c1c1",
      input: "#464646",
      muted: "#2c2c2c",
      mutedForeground: "#b7b7b7",
      pillForeground: "#c1c1c1",
      pillIcon: "#c1c1c1",
      pillSurfaceBorder: "#313131",
      pillSurfaceSelectedBorder: "#3a3a3a",
      popover: "#151515",
      popoverForeground: "#c1c1c1",
      prMerged: "#a27dfa",
      primary: "#c4c4c4",
      primaryForeground: "#1a1a1a",
      readbackForeground: "#a3a3a3",
      resourceSourceShelfCardHoverBorder: "#424242",
      ring: "#c4c4c4",
      secondary: "#282828",
      secondaryForeground: "#c1c1c1",
      shadowColor: "rgba(0, 0, 0, 0.4)",
      sidebar: "#1b1b1b",
      sidebarAccent: "#262626",
      sidebarAccentForeground: "#c1c1c1",
      sidebarBorder: "#2f2f2f",
      sidebarForeground: "#c1c1c1",
      sidebarRing: "#c4c4c4",
      sidebarSearchMatch: "#373123",
      sidebarSearchMatchBorder: "#50452b",
      stateActive: "rgba(193, 193, 193, 0.225)",
      stateHover: "rgba(193, 193, 193, 0.138)",
      subtleForeground: "#989898",
      success: "#4bc680",
      successForeground: "#d3b088",
      surfaceAttention: "rgba(240, 177, 53, 0.12)",
      surfaceDestructive: "rgba(204, 50, 61, 0.08)",
      surfaceDestructiveBorder: "rgba(204, 50, 61, 0.3)",
      surfaceRaised: "rgba(193, 193, 193, 0.025)",
      surfaceRaisedSolid: "#181818",
      surfaceRecessed: "rgba(193, 193, 193, 0.06)",
      surfaceRecessedSoftSolid: "#1b1b1b",
      surfaceRecessedSolid: "#1d1d1d",
      surfaceScrim: "rgba(21, 21, 21, 0.92)",
      surfaceSelected: "rgba(196, 196, 196, 0.12)",
      surfaceSelectedBorder: "rgba(196, 196, 196, 0.35)",
      timelineAccent: "#79a9db",
      versionUpgrade: "#b9b9b9",
      warning: "#fc8c45",
      warningText: "#fc8c45",
    },
  },
  dracula: {
    light: {
      accent: "#e4e5eb",
      accentForeground: "#282a36",
      ansi0: "#21222c",
      ansi1: "#ff5555",
      ansi10: "#69ff94",
      ansi11: "#ffffa5",
      ansi12: "#d6acff",
      ansi13: "#ff92df",
      ansi14: "#a4ffff",
      ansi15: "#ffffff",
      ansi2: "#50fa7b",
      ansi3: "#f1fa8c",
      ansi4: "#bd93f9",
      ansi5: "#ff79c6",
      ansi6: "#8be9fd",
      ansi7: "#f8f8f2",
      ansi8: "#6272a4",
      ansi9: "#ff6e6e",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#000000",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#000000",
      attention: "#9a7d00",
      background: "#f8f8f2",
      border: "#d6d7de",
      borderHairline: "#d4d6dd",
      borderSeam: "#e0e1e8",
      borderSeamVertical: "#e0e1e8",
      canvas: "#f8f8f2",
      card: "#f8f8f2",
      cardForeground: "#282a36",
      destructive: "#c4314b",
      destructiveForeground: "#ffffff",
      destructiveText: "#b3243d",
      diffAdded: "#2c7a4b",
      diffRemoved: "#c4314b",
      fileAccent: "#1f6f8b",
      foreground: "#282a36",
      ink: "#282a36",
      input: "#b3b5bd",
      muted: "#dddee5",
      mutedForeground: "#5f616c",
      pillForeground: "#282a36",
      pillIcon: "#282a36",
      pillSurfaceBorder: "#d6d7de",
      pillSurfaceSelectedBorder: "#caccd3",
      popover: "#f8f8f2",
      popoverForeground: "#282a36",
      prMerged: "#7d5bbe",
      primary: "#7d5bbe",
      primaryForeground: "#ffffff",
      readbackForeground: "#6b6d78",
      resourceSourceShelfCardHoverBorder: "#bfc1c9",
      ring: "#7d5bbe",
      secondary: "#e4e5eb",
      secondaryForeground: "#282a36",
      shadowColor: "rgba(51, 51, 51, 0.1)",
      sidebar: "#f1f2f8",
      sidebarAccent: "#e4e5eb",
      sidebarAccentForeground: "#282a36",
      sidebarBorder: "#d6d7de",
      sidebarForeground: "#282a36",
      sidebarRing: "#7d5bbe",
      sidebarSearchMatch: "#f3e9ce",
      sidebarSearchMatchBorder: "#efdfb6",
      stateActive: "rgba(40, 42, 54, 0.118)",
      stateHover: "rgba(40, 42, 54, 0.059)",
      subtleForeground: "#777983",
      success: "#2c7a4b",
      successForeground: "#184d5d",
      surfaceAttention: "rgba(154, 125, 0, 0.14)",
      surfaceDestructive: "rgba(196, 49, 75, 0.06)",
      surfaceDestructiveBorder: "rgba(196, 49, 75, 0.25)",
      surfaceRaised: "rgba(40, 42, 54, 0.025)",
      surfaceRaisedSolid: "#f2f2ed",
      surfaceRecessed: "rgba(40, 42, 54, 0.06)",
      surfaceRecessedSoftSolid: "#eceef4",
      surfaceRecessedSolid: "#eaeae6",
      surfaceScrim: "rgba(248, 248, 242, 0.92)",
      surfaceSelected: "rgba(125, 91, 190, 0.16)",
      surfaceSelectedBorder: "rgba(125, 91, 190, 0.35)",
      timelineAccent: "#1f6f8b",
      versionUpgrade: "#2f313d",
      warning: "#b8762e",
      warningText: "#8f5a22",
    },
    dark: {
      accent: "#3f414d",
      accentForeground: "#f8f8f2",
      ansi0: "#21222c",
      ansi1: "#ff5555",
      ansi10: "#69ff94",
      ansi11: "#ffffa5",
      ansi12: "#d6acff",
      ansi13: "#ff92df",
      ansi14: "#a4ffff",
      ansi15: "#ffffff",
      ansi2: "#50fa7b",
      ansi3: "#f1fa8c",
      ansi4: "#bd93f9",
      ansi5: "#ff79c6",
      ansi6: "#8be9fd",
      ansi7: "#f8f8f2",
      ansi8: "#6272a4",
      ansi9: "#ff6e6e",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#000000",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#000000",
      attention: "#f1fa8c",
      background: "#282a36",
      border: "#4b4d58",
      borderHairline: "#4e505b",
      borderSeam: "#3b3d49",
      borderSeamVertical: "#3b3d49",
      canvas: "#282a36",
      card: "#282a36",
      cardForeground: "#f8f8f2",
      destructive: "#ff5555",
      destructiveForeground: "#ffffff",
      destructiveText: "#ff7b7b",
      diffAdded: "#50fa7b",
      diffRemoved: "#ff5555",
      fileAccent: "#8be9fd",
      foreground: "#f8f8f2",
      ink: "#f8f8f2",
      input: "#646671",
      muted: "#444652",
      mutedForeground: "#b2b4bc",
      pillForeground: "#f8f8f2",
      pillIcon: "#f8f8f2",
      pillSurfaceBorder: "#4b4d58",
      pillSurfaceSelectedBorder: "#565863",
      popover: "#282a36",
      popoverForeground: "#f8f8f2",
      prMerged: "#bd93f9",
      primary: "#bd93f9",
      primaryForeground: "#282a36",
      readbackForeground: "#a5a7b0",
      resourceSourceShelfCardHoverBorder: "#5f616c",
      ring: "#bd93f9",
      secondary: "#3f414d",
      secondaryForeground: "#f8f8f2",
      shadowColor: "rgba(0, 0, 0, 0.4)",
      sidebar: "#2f313d",
      sidebarAccent: "#3d3f4b",
      sidebarAccentForeground: "#f8f8f2",
      sidebarBorder: "#484a56",
      sidebarForeground: "#f8f8f2",
      sidebarRing: "#bd93f9",
      sidebarSearchMatch: "#484440",
      sidebarSearchMatchBorder: "#5f5645",
      stateActive: "rgba(248, 248, 242, 0.225)",
      stateHover: "rgba(248, 248, 242, 0.138)",
      subtleForeground: "#989aa4",
      success: "#50fa7b",
      successForeground: "#b9fbc1",
      surfaceAttention: "rgba(241, 250, 140, 0.12)",
      surfaceDestructive: "rgba(255, 85, 85, 0.08)",
      surfaceDestructiveBorder: "rgba(255, 85, 85, 0.3)",
      surfaceRaised: "rgba(248, 248, 242, 0.025)",
      surfaceRaisedSolid: "#2c2e3a",
      surfaceRecessed: "rgba(248, 248, 242, 0.06)",
      surfaceRecessedSoftSolid: "#2f313d",
      surfaceRecessedSolid: "#323440",
      surfaceScrim: "rgba(40, 42, 54, 0.92)",
      surfaceSelected: "rgba(189, 147, 249, 0.12)",
      surfaceSelectedBorder: "rgba(189, 147, 249, 0.35)",
      timelineAccent: "#8be9fd",
      versionUpgrade: "#edeef4",
      warning: "#ffb86c",
      warningText: "#ffb86c",
    },
  },
  gruvbox: {
    light: {
      accent: "#eae1ba",
      accentForeground: "#3c3836",
      ansi0: "#3c3836",
      ansi1: "#cc241d",
      ansi10: "#b8bb26",
      ansi11: "#fabd2f",
      ansi12: "#83a598",
      ansi13: "#d3869b",
      ansi14: "#8ec07c",
      ansi15: "#fbf1c7",
      ansi2: "#98971a",
      ansi3: "#d79921",
      ansi4: "#458588",
      ansi5: "#b16286",
      ansi6: "#689d6a",
      ansi7: "#ebdbb2",
      ansi8: "#928374",
      ansi9: "#fb4934",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#ffffff",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#000000",
      ansiBgFg9: "#000000",
      attention: "#b57614",
      background: "#fbf1c7",
      border: "#ddd5b1",
      borderHairline: "#dcd3b0",
      borderSeam: "#e7deb8",
      borderSeamVertical: "#e7deb8",
      canvas: "#fbf1c7",
      card: "#fbf1c7",
      cardForeground: "#3c3836",
      destructive: "#cc241d",
      destructiveForeground: "#ffffff",
      destructiveText: "#9d0006",
      diffAdded: "#79740e",
      diffRemoved: "#9d0006",
      fileAccent: "#076678",
      foreground: "#3c3836",
      ink: "#3c3836",
      input: "#bdb699",
      muted: "#e4dbb6",
      mutedForeground: "#6e6b5e",
      pillForeground: "#3c3836",
      pillIcon: "#3c3836",
      pillSurfaceBorder: "#ddd5b1",
      pillSurfaceSelectedBorder: "#d2caa9",
      popover: "#fbf1c7",
      popoverForeground: "#3c3836",
      prMerged: "#8f3f71",
      primary: "#076678",
      primaryForeground: "#fbf1c7",
      readbackForeground: "#7a7666",
      resourceSourceShelfCardHoverBorder: "#c9c1a1",
      ring: "#076678",
      secondary: "#eae1ba",
      secondaryForeground: "#3c3836",
      shadowColor: "rgba(51, 51, 51, 0.1)",
      sidebar: "#f6edc3",
      sidebarAccent: "#eae1ba",
      sidebarAccentForeground: "#3c3836",
      sidebarBorder: "#ddd5b1",
      sidebarForeground: "#3c3836",
      sidebarRing: "#076678",
      sidebarSearchMatch: "#f5e4ad",
      sidebarSearchMatchBorder: "#f1da9b",
      stateActive: "rgba(60, 56, 54, 0.118)",
      stateHover: "rgba(60, 56, 54, 0.059)",
      subtleForeground: "#85816f",
      success: "#79740e",
      successForeground: "#55532f",
      surfaceAttention: "rgba(181, 118, 20, 0.14)",
      surfaceDestructive: "rgba(204, 36, 29, 0.06)",
      surfaceDestructiveBorder: "rgba(204, 36, 29, 0.25)",
      surfaceRaised: "rgba(60, 56, 54, 0.025)",
      surfaceRaisedSolid: "#f6ecc3",
      surfaceRecessed: "rgba(60, 56, 54, 0.06)",
      surfaceRecessedSoftSolid: "#f2e8c0",
      surfaceRecessedSolid: "#eee5be",
      surfaceScrim: "rgba(251, 241, 199, 0.92)",
      surfaceSelected: "rgba(7, 102, 120, 0.16)",
      surfaceSelectedBorder: "rgba(7, 102, 120, 0.35)",
      timelineAccent: "#076678",
      versionUpgrade: "#413f3a",
      warning: "#d65d0e",
      warningText: "#af3a03",
    },
    dark: {
      accent: "#3e3c38",
      accentForeground: "#ebdbb2",
      ansi0: "#3c3836",
      ansi1: "#cc241d",
      ansi10: "#b8bb26",
      ansi11: "#fabd2f",
      ansi12: "#83a598",
      ansi13: "#d3869b",
      ansi14: "#8ec07c",
      ansi15: "#fbf1c7",
      ansi2: "#98971a",
      ansi3: "#d79921",
      ansi4: "#458588",
      ansi5: "#b16286",
      ansi6: "#689d6a",
      ansi7: "#ebdbb2",
      ansi8: "#928374",
      ansi9: "#fb4934",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#ffffff",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#000000",
      ansiBgFg9: "#000000",
      attention: "#fabd2f",
      background: "#282828",
      border: "#494740",
      borderHairline: "#4c4942",
      borderSeam: "#3a3936",
      borderSeamVertical: "#3a3936",
      canvas: "#282828",
      card: "#282828",
      cardForeground: "#ebdbb2",
      destructive: "#fb4934",
      destructiveForeground: "#ffffff",
      destructiveText: "#fb4934",
      diffAdded: "#b8bb26",
      diffRemoved: "#fb4934",
      fileAccent: "#83a598",
      foreground: "#ebdbb2",
      ink: "#ebdbb2",
      input: "#615d51",
      muted: "#43413c",
      mutedForeground: "#aba085",
      pillForeground: "#ebdbb2",
      pillIcon: "#ebdbb2",
      pillSurfaceBorder: "#494740",
      pillSurfaceSelectedBorder: "#535048",
      popover: "#282828",
      popoverForeground: "#ebdbb2",
      prMerged: "#d3869b",
      primary: "#83a598",
      primaryForeground: "#282828",
      readbackForeground: "#9f957d",
      resourceSourceShelfCardHoverBorder: "#5c584e",
      ring: "#83a598",
      secondary: "#3e3c38",
      secondaryForeground: "#ebdbb2",
      shadowColor: "rgba(0, 0, 0, 0.4)",
      sidebar: "#2f2f2d",
      sidebarAccent: "#3c3b37",
      sidebarAccentForeground: "#ebdbb2",
      sidebarBorder: "#47443f",
      sidebarForeground: "#ebdbb2",
      sidebarRing: "#83a598",
      sidebarSearchMatch: "#494233",
      sidebarSearchMatchBorder: "#5f5439",
      stateActive: "rgba(235, 219, 178, 0.225)",
      stateHover: "rgba(235, 219, 178, 0.138)",
      subtleForeground: "#928a74",
      success: "#b8bb26",
      successForeground: "#dacb7b",
      surfaceAttention: "rgba(250, 189, 47, 0.12)",
      surfaceDestructive: "rgba(251, 73, 52, 0.08)",
      surfaceDestructiveBorder: "rgba(251, 73, 52, 0.3)",
      surfaceRaised: "rgba(235, 219, 178, 0.025)",
      surfaceRaisedSolid: "#2c2c2b",
      surfaceRecessed: "rgba(235, 219, 178, 0.06)",
      surfaceRecessedSoftSolid: "#2f2e2d",
      surfaceRecessedSolid: "#32312f",
      surfaceScrim: "rgba(40, 40, 40, 0.92)",
      surfaceSelected: "rgba(131, 165, 152, 0.12)",
      surfaceSelectedBorder: "rgba(131, 165, 152, 0.35)",
      timelineAccent: "#83a598",
      versionUpgrade: "#e2d3ac",
      warning: "#fe8019",
      warningText: "#fe8019",
    },
  },
  nord: {
    light: {
      accent: "#dbdee4",
      accentForeground: "#2e3440",
      ansi0: "#3b4252",
      ansi1: "#bf616a",
      ansi10: "#a3be8c",
      ansi11: "#ebcb8b",
      ansi12: "#81a1c1",
      ansi13: "#b48ead",
      ansi14: "#8fbcbb",
      ansi15: "#eceff4",
      ansi2: "#a3be8c",
      ansi3: "#ebcb8b",
      ansi4: "#81a1c1",
      ansi5: "#b48ead",
      ansi6: "#88c0d0",
      ansi7: "#e5e9f0",
      ansi8: "#4c566a",
      ansi9: "#bf616a",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#000000",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#000000",
      attention: "#ebcb8b",
      background: "#eceff4",
      border: "#cfd2d9",
      borderHairline: "#cdd1d7",
      borderSeam: "#d8dbe1",
      borderSeamVertical: "#d8dbe1",
      canvas: "#eceff4",
      card: "#eceff4",
      cardForeground: "#2e3440",
      destructive: "#bf616a",
      destructiveForeground: "#ffffff",
      destructiveText: "#a1343d",
      diffAdded: "#5e8a52",
      diffRemoved: "#bf616a",
      fileAccent: "#5e81ac",
      foreground: "#2e3440",
      ink: "#2e3440",
      input: "#afb3bb",
      muted: "#d5d8de",
      mutedForeground: "#616772",
      pillForeground: "#2e3440",
      pillIcon: "#2e3440",
      pillSurfaceBorder: "#cfd2d9",
      pillSurfaceSelectedBorder: "#c4c8cf",
      popover: "#eceff4",
      popoverForeground: "#2e3440",
      prMerged: "#9d7cb8",
      primary: "#5e81ac",
      primaryForeground: "#eceff4",
      readbackForeground: "#6c727c",
      resourceSourceShelfCardHoverBorder: "#babec5",
      ring: "#5e81ac",
      secondary: "#dbdee4",
      secondaryForeground: "#2e3440",
      shadowColor: "rgba(51, 51, 51, 0.1)",
      sidebar: "#e8eaf0",
      sidebarAccent: "#dbdee4",
      sidebarAccentForeground: "#2e3440",
      sidebarBorder: "#cfd2d9",
      sidebarForeground: "#2e3440",
      sidebarRing: "#5e81ac",
      sidebarSearchMatch: "#eae3d0",
      sidebarSearchMatchBorder: "#e8dab8",
      stateActive: "rgba(46, 52, 64, 0.118)",
      stateHover: "rgba(46, 52, 64, 0.059)",
      subtleForeground: "#787d87",
      success: "#6f9655",
      successForeground: "#2b6268",
      surfaceAttention: "rgba(235, 203, 139, 0.14)",
      surfaceDestructive: "rgba(191, 97, 106, 0.06)",
      surfaceDestructiveBorder: "rgba(191, 97, 106, 0.25)",
      surfaceRaised: "rgba(46, 52, 64, 0.025)",
      surfaceRaisedSolid: "#e7eaef",
      surfaceRecessed: "rgba(46, 52, 64, 0.06)",
      surfaceRecessedSoftSolid: "#e3e6ec",
      surfaceRecessedSolid: "#dfe3e8",
      surfaceScrim: "rgba(236, 239, 244, 0.92)",
      surfaceSelected: "rgba(94, 129, 172, 0.16)",
      surfaceSelectedBorder: "rgba(94, 129, 172, 0.35)",
      timelineAccent: "#5e81ac",
      versionUpgrade: "#343a46",
      warning: "#d08770",
      warningText: "#99543a",
    },
    dark: {
      accent: "#414854",
      accentForeground: "#d8dee9",
      ansi0: "#3b4252",
      ansi1: "#bf616a",
      ansi10: "#a3be8c",
      ansi11: "#ebcb8b",
      ansi12: "#81a1c1",
      ansi13: "#b48ead",
      ansi14: "#8fbcbb",
      ansi15: "#eceff4",
      ansi2: "#a3be8c",
      ansi3: "#ebcb8b",
      ansi4: "#81a1c1",
      ansi5: "#b48ead",
      ansi6: "#88c0d0",
      ansi7: "#e5e9f0",
      ansi8: "#4c566a",
      ansi9: "#bf616a",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#000000",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#000000",
      attention: "#ebcb8b",
      background: "#2e3440",
      border: "#4b525e",
      borderHairline: "#4e5460",
      borderSeam: "#3e4451",
      borderSeamVertical: "#3e4451",
      canvas: "#2e3440",
      card: "#2e3440",
      cardForeground: "#d8dee9",
      destructive: "#bf616a",
      destructiveForeground: "#ffffff",
      destructiveText: "#d6868d",
      diffAdded: "#a3be8c",
      diffRemoved: "#bf616a",
      fileAccent: "#88c0d0",
      foreground: "#d8dee9",
      ink: "#d8dee9",
      input: "#616773",
      muted: "#464c58",
      mutedForeground: "#a1a7b3",
      pillForeground: "#d8dee9",
      pillIcon: "#d8dee9",
      pillSurfaceBorder: "#4b525e",
      pillSurfaceSelectedBorder: "#555b67",
      popover: "#2e3440",
      popoverForeground: "#d8dee9",
      prMerged: "#b48ead",
      primary: "#88c0d0",
      primaryForeground: "#2e3440",
      readbackForeground: "#969ca8",
      resourceSourceShelfCardHoverBorder: "#5c626f",
      ring: "#88c0d0",
      secondary: "#414854",
      secondaryForeground: "#d8dee9",
      shadowColor: "rgba(0, 0, 0, 0.4)",
      sidebar: "#343a46",
      sidebarAccent: "#404652",
      sidebarAccentForeground: "#d8dee9",
      sidebarBorder: "#494f5c",
      sidebarForeground: "#d8dee9",
      sidebarRing: "#88c0d0",
      sidebarSearchMatch: "#4e4d48",
      sidebarSearchMatchBorder: "#645e4c",
      stateActive: "rgba(216, 222, 233, 0.225)",
      stateHover: "rgba(216, 222, 233, 0.138)",
      subtleForeground: "#8c929e",
      success: "#a3be8c",
      successForeground: "#c1d1b4",
      surfaceAttention: "rgba(235, 203, 139, 0.12)",
      surfaceDestructive: "rgba(191, 97, 106, 0.08)",
      surfaceDestructiveBorder: "rgba(191, 97, 106, 0.3)",
      surfaceRaised: "rgba(216, 222, 233, 0.025)",
      surfaceRaisedSolid: "#323844",
      surfaceRecessed: "rgba(216, 222, 233, 0.06)",
      surfaceRecessedSoftSolid: "#343a46",
      surfaceRecessedSolid: "#373d49",
      surfaceScrim: "rgba(46, 52, 64, 0.92)",
      surfaceSelected: "rgba(136, 192, 208, 0.12)",
      surfaceSelectedBorder: "rgba(136, 192, 208, 0.35)",
      timelineAccent: "#88c0d0",
      versionUpgrade: "#d1d6e2",
      warning: "#d08770",
      warningText: "#e0a48f",
    },
  },
  solarized: {
    light: {
      accent: "#e9e6d1",
      accentForeground: "#073642",
      ansi0: "#073642",
      ansi1: "#dc322f",
      ansi10: "#93a1a1",
      ansi11: "#839496",
      ansi12: "#657b83",
      ansi13: "#6c71c4",
      ansi14: "#2aa198",
      ansi15: "#fdf6e3",
      ansi2: "#859900",
      ansi3: "#b58900",
      ansi4: "#268bd2",
      ansi5: "#d33682",
      ansi6: "#2aa198",
      ansi7: "#eee8d5",
      ansi8: "#586e75",
      ansi9: "#cb4b16",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#ffffff",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#ffffff",
      attention: "#b58900",
      background: "#fdf6e3",
      border: "#d9dac3",
      borderHairline: "#d7d8c2",
      borderSeam: "#e5e3cd",
      borderSeamVertical: "#e5e3cd",
      canvas: "#fdf6e3",
      card: "#fdf6e3",
      cardForeground: "#073642",
      destructive: "#dc322f",
      destructiveForeground: "#ffffff",
      destructiveText: "#c12321",
      diffAdded: "#718c00",
      diffRemoved: "#dc322f",
      fileAccent: "#268bd2",
      foreground: "#073642",
      ink: "#073642",
      input: "#b0bba5",
      muted: "#e1e0ca",
      mutedForeground: "#496e67",
      pillForeground: "#073642",
      pillIcon: "#073642",
      pillSurfaceBorder: "#d9dac3",
      pillSurfaceSelectedBorder: "#cbcfb9",
      popover: "#fdf6e3",
      popoverForeground: "#073642",
      prMerged: "#6c71c4",
      primary: "#268bd2",
      primaryForeground: "#fdf6e3",
      readbackForeground: "#577a6f",
      resourceSourceShelfCardHoverBorder: "#bfc6af",
      ring: "#268bd2",
      secondary: "#e9e6d1",
      secondaryForeground: "#073642",
      shadowColor: "rgba(51, 51, 51, 0.1)",
      sidebar: "#f7f1de",
      sidebarAccent: "#e9e6d1",
      sidebarAccentForeground: "#073642",
      sidebarBorder: "#d9dac3",
      sidebarForeground: "#073642",
      sidebarRing: "#268bd2",
      sidebarSearchMatch: "#f6e7c2",
      sidebarSearchMatchBorder: "#f2deac",
      stateActive: "rgba(7, 54, 66, 0.118)",
      stateHover: "rgba(7, 54, 66, 0.059)",
      subtleForeground: "#668577",
      success: "#718c00",
      successForeground: "#006250",
      surfaceAttention: "rgba(181, 137, 0, 0.14)",
      surfaceDestructive: "rgba(220, 50, 47, 0.06)",
      surfaceDestructiveBorder: "rgba(220, 50, 47, 0.25)",
      surfaceRaised: "rgba(7, 54, 66, 0.025)",
      surfaceRaisedSolid: "#f6f1df",
      surfaceRecessed: "rgba(7, 54, 66, 0.06)",
      surfaceRecessedSoftSolid: "#f2edd9",
      surfaceRecessedSolid: "#ede9d9",
      surfaceScrim: "rgba(253, 246, 227, 0.92)",
      surfaceSelected: "rgba(38, 139, 210, 0.16)",
      surfaceSelectedBorder: "rgba(38, 139, 210, 0.35)",
      timelineAccent: "#268bd2",
      versionUpgrade: "#103d47",
      warning: "#cb4b16",
      warningText: "#a53c12",
    },
    dark: {
      accent: "#153943",
      accentForeground: "#93a1a1",
      ansi0: "#073642",
      ansi1: "#dc322f",
      ansi10: "#93a1a1",
      ansi11: "#839496",
      ansi12: "#657b83",
      ansi13: "#6c71c4",
      ansi14: "#2aa198",
      ansi15: "#fdf6e3",
      ansi2: "#859900",
      ansi3: "#b58900",
      ansi4: "#268bd2",
      ansi5: "#d33682",
      ansi6: "#2aa198",
      ansi7: "#eee8d5",
      ansi8: "#586e75",
      ansi9: "#cb4b16",
      ansiBgFg0: "#ffffff",
      ansiBgFg1: "#ffffff",
      ansiBgFg10: "#000000",
      ansiBgFg11: "#000000",
      ansiBgFg12: "#000000",
      ansiBgFg13: "#000000",
      ansiBgFg14: "#000000",
      ansiBgFg15: "#000000",
      ansiBgFg2: "#000000",
      ansiBgFg3: "#000000",
      ansiBgFg4: "#000000",
      ansiBgFg5: "#000000",
      ansiBgFg6: "#000000",
      ansiBgFg7: "#000000",
      ansiBgFg8: "#ffffff",
      ansiBgFg9: "#ffffff",
      attention: "#b58900",
      background: "#002b36",
      border: "#1f404a",
      borderHairline: "#21424c",
      borderSeam: "#123741",
      borderSeamVertical: "#123741",
      canvas: "#002b36",
      card: "#002b36",
      cardForeground: "#93a1a1",
      destructive: "#dc322f",
      destructiveForeground: "#ffffff",
      destructiveText: "#e8645f",
      diffAdded: "#859900",
      diffRemoved: "#dc322f",
      fileAccent: "#2aa198",
      foreground: "#93a1a1",
      ink: "#93a1a1",
      input: "#324f58",
      muted: "#1a3c46",
      mutedForeground: "#677b82",
      pillForeground: "#93a1a1",
      pillIcon: "#93a1a1",
      pillSurfaceBorder: "#1f404a",
      pillSurfaceSelectedBorder: "#274650",
      popover: "#002b36",
      popoverForeground: "#93a1a1",
      prMerged: "#6c71c4",
      primary: "#268bd2",
      primaryForeground: "#002b36",
      readbackForeground: "#5e747b",
      resourceSourceShelfCardHoverBorder: "#2e4c55",
      ring: "#268bd2",
      secondary: "#153943",
      secondaryForeground: "#93a1a1",
      shadowColor: "rgba(0, 0, 0, 0.4)",
      sidebar: "#06303a",
      sidebarAccent: "#143842",
      sidebarAccentForeground: "#93a1a1",
      sidebarBorder: "#1d3f49",
      sidebarForeground: "#93a1a1",
      sidebarRing: "#268bd2",
      sidebarSearchMatch: "#304641",
      sidebarSearchMatchBorder: "#4b5846",
      stateActive: "rgba(147, 161, 161, 0.225)",
      stateHover: "rgba(147, 161, 161, 0.138)",
      subtleForeground: "#556c74",
      success: "#859900",
      successForeground: "#919d67",
      surfaceAttention: "rgba(181, 137, 0, 0.12)",
      surfaceDestructive: "rgba(220, 50, 47, 0.08)",
      surfaceDestructiveBorder: "rgba(220, 50, 47, 0.3)",
      surfaceRaised: "rgba(147, 161, 161, 0.025)",
      surfaceRaisedSolid: "#042e38",
      surfaceRecessed: "rgba(147, 161, 161, 0.06)",
      surfaceRecessedSoftSolid: "#062f3a",
      surfaceRecessedSolid: "#09313c",
      surfaceScrim: "rgba(0, 43, 54, 0.92)",
      surfaceSelected: "rgba(38, 139, 210, 0.12)",
      surfaceSelectedBorder: "rgba(38, 139, 210, 0.35)",
      timelineAccent: "#2aa198",
      versionUpgrade: "#8d9ba0",
      warning: "#cb4b16",
      warningText: "#e07a4e",
    },
  },
};

/** `--radius` and the Tailwind `--radius-*` steps, in CSS pixels. */
export const nativeRadii = {
  base: 8,
  sm: 4,
  md: 6,
  lg: 8,
  xl: 12,
};

export interface NativeTextStyle {
  fontSize: number;
  lineHeight: number;
}

/**
 * The `--text-*` scale theme.css overrides, using the coarse-pointer (touch)
 * values as the base, in CSS pixels. Sizes theme.css does not override keep
 * Tailwind's defaults.
 */
export const nativeTypography = {
  "2xs": {
    fontSize: 11,
    lineHeight: 15,
  },
  xs: {
    fontSize: 14,
    lineHeight: 20,
  },
  sm: {
    fontSize: 15,
    lineHeight: 22,
  },
  base: {
    fontSize: 16,
    lineHeight: 24,
  },
} satisfies Record<string, NativeTextStyle>;

export type NativeTextSize = keyof typeof nativeTypography;
